...
import { mockFaq } from "../src/lib/airtable-mock";
...
async function main() {
  ...
  const existing = await base("FAQ").select().all();
  if (existing.length > 0) {
    ...
    console.log(`✓ Usunięto poprzednie FAQ (${existing.length})`);
  }
  ...
  console.log(`✓ Zapisano ${mockFaq.length} pytań FAQ`);
}
